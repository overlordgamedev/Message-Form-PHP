...
<body>
    ...
    $sql = "SELECT email, username, message, created_at, user_ip, user_browser FROM messages ORDER BY created_at DESC";
    $stmt = $pdo->query($sql);
    ...
    if ($stmt->rowCount() > 0) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            echo "<strong>IP Адрес:</strong> " . nl2br(htmlspecialchars($row['user_ip'])) . "<br>";
            echo "<strong>Информаци о браузере:</strong> " . nl2br(htmlspecialchars($row['user_browser'])) . "<br>";
